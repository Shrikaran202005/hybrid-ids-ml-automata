...
LABELMAP_PATH = "models/label_map.pkl"
...
# Encode labels
y_raw = df["label"].astype(str).fillna("BENIGN")
le = LabelEncoder()
...
if os.path.exists(MODEL_PATH):
    st.success("✅ Found pretrained model! Loading it...")
    model = IntrusionNet(input_dim=X.shape[1], num_classes=len(label_map))
    model.load_state_dict(torch.load(MODEL_PATH, map_location=device))
    model.to(device)
else:
    st.warning("⚙ No model found. Training a new one...")
    ...
    # Train model
    model = IntrusionNet(input_dim=X_train_res.shape[1], num_classes=len(label_map)).to(device)
